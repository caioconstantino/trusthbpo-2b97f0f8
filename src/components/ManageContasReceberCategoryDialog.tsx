...
interface ManageContasReceberCategoryDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSelectCategory: (category: string) => void;
}

export const ManageContasReceberCategoryDialog = ({
  open,
  onOpenChange,
  onSelectCategory,
}: ManageContasReceberCategoryDialogProps) => {
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      ...
      const { error } = await supabase.from("tb_categorias_contas_receber").insert({
        nome: nome.toUpperCase(),
        dominio,
        parent_id: tipo === "subcategoria" ? parseInt(parentId) : null,
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Cadastrar Categoria de Receita</DialogTitle>
        </DialogHeader>
