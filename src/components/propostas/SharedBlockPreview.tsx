...
import { PropostaBlock, BlockConfig, OfertaTier, PrazoItem, AssinaturaParty } from "@/hooks/usePropostas";
...
export const BLOCK_GROUPS = ["Layout", "Conteúdo", "Mídia", "Finalização"];
...
export function PropertiesPanel({ block, onConfigChange, onContentChange }: {
  block: PropostaBlock;
...
export function ElementPalette({ onAddBlock }: { onAddBlock: (type: PropostaBlock["type"]) => void }) {
  return (
                  ...
                  <TooltipProvider key={type} delayDuration={300}>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button onClick={() => onAddBlock(type)} className="w-full flex items-center gap-2 px-2 py-1.5 rounded-md text-left hover:bg-muted transition-colors group">
